...
const calculateSizeOfCanvas = (size: number) => {
    return closestNumber(size - ((size / 100) * 10), 5);
}; 

const GameOfLifePage: FunctionComponent = () => {
    const [canvasWidth, setCanvasWidth] = useState<number>(calculateSizeOfCanvas(window.innerWidth));
    const [canvasHeight, setCanvasHeight] = useState<number>(calculateSizeOfCanvas(window.innerHeight));
        ...
        <div style={rootDivStyle}>
            <div style={{display: 'flex',justifyContent:'center',height: '100%'}}>
                <div style={{display: 'flex',alignItems: 'center'}}>
                    <canvas ref={canvasRef} style={{border: '1px solid black', backgroundColor: 'white'}} width={canvasWidth} height={canvasHeight}/>
                </div>
            </div>
        </div>
    );
